...
BEGIN
  ...
  BEGIN TRY
    ...
    BEGIN
      SELECT @Return_Code = 2,
             @Error       = 'st_Gerenciar_Cliente: ID obrigatório.';
      RAISERROR(@Error, 16, 1);
    END
    ...
    BEGIN
      ...
      BEGIN
        SELECT @Return_Code = 3,
               @Error       = 'st_Gerenciar_Cliente: Cliente não encontrado.';
        RAISERROR(@Error, 16, 1);
      END
    END
    ...
    BEGIN
      ...
      BEGIN
        SELECT @Return_Code = 2,
               @Error       = 'st_Gerenciar_Cliente: Tipo de pessoa inválido. Valores: F=Física, J=Jurídica.';
        RAISERROR(@Error, 16, 1);
      END
      ...
      BEGIN
        SELECT @Return_Code = 2,
               @Error       = 'st_Gerenciar_Cliente: Nome obrigatório (mínimo 3 caracteres).';
        RAISERROR(@Error, 16, 1);
      END
      ...
      BEGIN
        ...
        BEGIN
          SELECT @Return_Code = 2,
                 @Error       = 'st_Gerenciar_Cliente: CPF inválido. Deve conter 11 dígitos numéricos.';
          RAISERROR(@Error, 16, 1);
        END
      ...
      BEGIN
        ...
        BEGIN
          SELECT @Return_Code = 2,
                 @Error       = 'st_Gerenciar_Cliente: CNPJ inválido. Deve conter 14 dígitos numéricos.';
          RAISERROR(@Error, 16, 1);
        END
        ...
        BEGIN
          SELECT @Return_Code = 2,
                 @Error       = 'st_Gerenciar_Cliente: CNPJ já cadastrado.';
          RAISERROR(@Error, 16, 1);
        END
      END
      ...
      BEGIN
        SELECT @Return_Code = 2,
               @Error       = 'st_Gerenciar_Cliente: E-mail inválido.';
        RAISERROR(@Error, 16, 1);
      END
    END
    ...
    IF (@Acao = 'C')
    BEGIN
      INSERT INTO dbo.Clientes (TipoPessoa,
                                Nome,
                                NomeFantasia,
                                CPF,
                                CNPJ,
